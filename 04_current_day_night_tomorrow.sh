...
ICON_CUR=$(printf "%02d" $(echo "$WEATHER_DATA" | grep '<svg class="weather-icon"' | awk -F'weathericons/v2a/' '{print $2}' | cut -f 1 -d "."))
ICON_D=$(printf "%02d\n" $(echo "${DATA_TODAY[0]}" | grep 'svg class="icon'  | awk -F'weathericons/v2a/' '{print $2}' | cut -f 1 -d "."))
ICON_N=$(printf "%02d\n" $(echo "${DATA_TODAY[1]}" | grep 'svg class="icon'  | awk -F'weathericons/v2a/' '{print $2}' | cut -f 1 -d "."))
ICON_T=$(printf "%02d" $(echo "${DATA_TOMORROW[0]}" |  grep 'svg class="icon' | awk -F'weathericons/v2a/' '{print $2}' | cut -f 1 -d "."))
