...
icon_sat=`echo "$weather_weekend" | grep -e 'dailyForecast' | tr '{|}' '\n' | grep -B1 -m1 'Saturday' | sed -n 1p | sed -e 's/.*\"icon\":\([0-9]*\).*/\1/' | tr -cd '0123456789' | awk '{printf "%02d", $1}'`
icon_sat=`printf "%.2d\n" $icon_sat`
icon_sun=`echo "$weather_weekend" | grep -e 'dailyForecast' | tr '{|}' '\n' | grep -B1 -m1 'Sunday' | sed -n 1p | sed -e 's/.*\"icon\":\([0-9]*\).*/\1/' | tr -cd '0123456789' | awk '{printf "%02d", $1}'`
icon_sun=`printf "%.2d\n" $icon_sun`
