...
if [ $F_ICON -eq 1 ]; then
  for (( i = 0; i < $NUM_L; ++i ))
  do
  ICON_WEEK[$i]=$(printf "%02d" $(echo "${DATA_WEEK[$(expr $i + $AFTER)]}" | grep 'class="weather-icon icon "' | awk -F'weathericons/' '{print $2}' | cut -f 1 -d "." ))
